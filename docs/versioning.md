...
| **3.2.0** | **2026-08-07** | **MINOR** | 平台发布配置（博客园试点）：默认参数 + 选项源缓存 + 草稿/发布 |
| **3.2.1** | **2026-08-07** | **PATCH** | 文档元数据（FM）→ 平台发布参数快照；博客园合集映射；构建旁路 JS 遮蔽修复 |
| **3.3.0** | **2026-08-08** | **MINOR** | 51CTO 发布配置（草稿/发布）+ 分类上次操作缓存；博客园题图 / 腾讯云 ID / 掘金 FM 标签修复 |

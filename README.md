...
- **平台限制标注**: 平台表标注标题/正文上限与草稿策略（如小红书长文标题≤64、正文≤约1万字超限截断）
...
| 平台 | ID | 类型 | Markdown | 限制 | 状态 |
|-----|-----|-----|-----|-----|-----|
| 微信公众号 | weixin | 主流自媒体 | 🟡 部分 | 标题≤64 / 摘要≤120；正文视频≤3（平台校验） | ✅ |
| 知乎 | zhihu | 主流自媒体 | ✅ 完整 | — | ✅ |
| 微博 | weibo | 主流自媒体 | ⚪ 弱 | — | ✅ |
| 小红书 | xiaohongshu | 主流自媒体 | ⚪ 弱 | 标题≤64 / 正文≤约1万字；仅长文草稿；⚠️ 图片待下版本修复 | ✅ |
| 掘金 | juejin | 技术社区 | ✅ 完整 | — | ✅ |
| CSDN | csdn | 技术社区 | ✅ 完整 | — | ✅ |
| 简书 | jianshu | 通用 | ✅ 完整 | — | ✅ 🆕 |
| 头条号 | toutiao | 通用 | 🟡 部分 | — | ✅ |
...
| 大鱼号 | dayu | 通用 | 🟡 部分 | — | ✅ 🆕 |
| 一点号 | yidian | 通用 | 🟡 部分 | — | ✅ |
| 虎扑 | hupu | 体育社区 | 🟡 部分 | 无草稿（直接发帖）；标题 4–40 字 | ✅ 🆕 |
| 美篇 | meipian | 图文创作 | 🟡 部分 | 无草稿箱，同步后公开；⚠️ 图片待下版本修复 | ✅ 🆕 |
| 企鹅号 | qiehao | 通用 | 🟡 部分 | 标题 5–64 字（过短报错 / 超长截断）；仅草稿 | ✅ 🆕 |
| 51CTO | 51cto | 技术社区 | ✅ 完整 | 强制仅草稿 | ✅ |
| 慕课网 | imooc | 技术社区 | ✅ 完整 | 强制仅草稿 | ✅ |
| 开源中国 | oschina | 技术社区 | ✅ 完整 | 强制仅草稿 | ✅ |
| SegmentFault | segmentfault | 技术社区 | ✅ 完整 | 强制仅草稿 | ✅ |
| 博客园 | cnblogs | 技术社区 | ✅ 完整 | — | ✅ |
| 搜狐焦点 | sohufocus | 房产 | 🟡 部分 | — | ✅ |
| X (Twitter) | x | 海外 | ⚪ 弱 | — | ✅ |
| Reddit | reddit | 海外 | ✅ 完整 | 固定个人 profile；GraphQL RICHTEXT 草稿；**图片暂关闭**（仅文字） | ✅ 🆕 |
...
>
> **限制**：适配器内已实现或明确映射的平台约束（字数截断/校验、强制草稿、无草稿直接发布等）。「—」表示适配器未做额外字数截断或特殊发布策略（仍受各平台自身审核与接口约束；多数平台默认同步为草稿）。
>
...
默认策略是同步为**草稿**，由你确认后再发布。下列为适配器内已实现或明确映射的约束（字数、草稿策略等）：
...
| **虎扑** | 无草稿接口。同步后直接发帖到步行街（即时可见）；标题需 4–40 字（过短报错 / 超长截断）。 |
| **美篇** | 无独立草稿箱。同步后直接以**公开**状态发布（不会保存为「仅作者可见 / 私密」）。内容为段落式图文，Markdown 仅部分保留。登录态在页面 `localStorage`，需先在浏览器打开 [美篇编辑器](https://www.meipian.cn/editor) 并完成登录。**已知问题**：图片上传/转存尚不稳定，将在下版本修复；本版本建议纯文本或同步后手工补图。 |
| **企鹅号** | 标题需 5–64 字（过短报错 / 超长截断）；仅保存草稿，不调用发布接口。 |
| **51CTO / 慕课网 / 开源中国 / SegmentFault** | 适配器强制仅草稿（忽略「直接发布」选项）。 |

其余平台仍遵循「草稿优先」，适配器未做额外字数截断（仍受各平台自身审核与接口约束）；同步完成后请在各平台后台确认再正式公开。

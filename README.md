...
- **图片自动上传**: 自动将文章中的 **http(s) 外链图片转存**到目标平台图床（**仅千帆不支持外链中转**）；本地图多数平台支持（腾讯云/火山本版本不支持本地图；千帆本地图亦不支持）。详见[平台限制](#平台限制)
...
>
> **发布**：`草稿` = 默认同步为草稿（含强制仅草稿）；`直接发布` = 同步后即时公开（虎扑、美篇）。
>
...
| 能力 | 说明 |
|------|------|
| **外链图片中转**（http(s) → 平台图床） | **仅千帆不支持**；其余平台均支持外链转存。 |
| **本地图片**（导入 Markdown / data URI） | 多数平台支持。例外：千帆不支持；腾讯云社区 / 火山引擎本版本不支持（同步前剥离）。 |
...
| **千帆** | 草稿同步。**当前唯一不支持外链图片中转的平台**；本地图亦不支持（data URI 剥离；http(s) 不转存）。建议同步后在编辑器手工补图。 |
...
- 📝 文档明确：外链图片中转**仅千帆不支持**，其余平台均支持
- 🔧 腾讯云社区 / 火山引擎：恢复 **http(s) 外链图片转存**；本地图仍不支持（data URI 剥离）
- ⚠️ 千帆社区：本地图与外链中转均不支持（当前唯一关闭外链中转）
